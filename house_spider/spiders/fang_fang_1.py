...
class FangFangSpider(scrapy.Spider):
    ...
    def start_requests(self):
        sql = 'select * from fang_city limit 2,1'
        ...
        for row in data:
            if row['new_house_url']:
                print(row['new_house_url'])
    ...
    def parse(self, response):
        ...
        for i in range(1, max_total_page):
            request_url = "http://newhouse.sz.fang.com/house/s/b9" + str(i)
    ...
    def parse_list(self, response):
        ...
        for url_single in url_list:
            yield scrapy.Request(url_single.rstrip('/'), callback=self.parse_detail_url)
